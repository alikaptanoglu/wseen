...
import 'package:wseen/models/date.dart';
...
class _LogCardState extends State<LogCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<ModelProvider>(
      builder: (context, value, child) {
        ...
        stream: FirebaseFirestore.instance.collection('webcontacts').doc(widget.number).collection('logs').where('start',isGreaterThanOrEqualTo: value.startDate, isLessThanOrEqualTo: value.endDate).orderBy('start', descending: true).snapshots(),
            ...
            if (snapshot.data!.docs.isNotEmpty) {
              return Column(
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(color: Colors.transparent, borderRadius: BorderRadius.circular(Values.contactCardRadius / 2), border: Border.all(color: isOnline ? Colors.green : Colors.grey, width: 1)),
                  child: Row(
  ...
  Widget _startHour(DateTime start) {
    return Align(
      alignment: Alignment.centerLeft,
      child: Text(
  ...
  Widget _startDate(DateTime start) {
    return Align(
      alignment: Alignment.centerLeft,
      child: Text(
  ...
  Widget _duration(bool isOnline, DateTime start, Duration diff) {
    return Container(child: isOnline ? _sayac(start) : Text(printDuration(diff), style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 16)));
  }

  Widget _textDuration() {
    return Text('duration', style: TextStyle(color: Colors.white.withOpacity(0.6),fontSize: 16));
  }

  Widget _endHour(bool isOnline, DateTime end) {
    return Align(
      alignment: Alignment.centerRight,
      child: isOnline
          ? null
  ...
  Widget _endDate(bool isOnline, DateTime end) {
    return isOnline
        ? Row(
